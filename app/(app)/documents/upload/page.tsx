...
export default function UploadPage() {
  ...
  const quotaLimit = quota?.quota_limit_bytes ?? 2 * 1024 * 1024; // fallback 2 Mo
  const usedBytes = quota?.used_bytes ?? 0;
  const percent = Math.min((usedBytes / quotaLimit) * 100, 100);
  const quotaExceeded = usedBytes >= quotaLimit;
  ...
  async function handleUpload() {
    ...
    if (quotaExceeded) {
      setStatus("Quota dépassé : supprimez des documents pour continuer.");
      return;
    }
      ...
      <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        <div className="mb-6 rounded-lg bg-blue-50 p-4 text-sm text-slate-700">
          ...
          {quotaExceeded && (
            <p className="mt-2 text-red-600 font-semibold">
              Quota dépassé : supprimez des documents pour continuer.
            </p>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full bg-blue-700 text-white p-3 rounded-lg font-semibold hover:bg-blue-800 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
